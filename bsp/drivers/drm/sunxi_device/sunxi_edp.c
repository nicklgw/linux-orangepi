...
s32 edp_core_enhance_frame_enable(struct sunxi_edp_hw_desc *edp_hw, bool enable)
{
	struct sunxi_edp_hw_video_ops *ops = edp_hw->video_ops;
	s32 ret = RET_OK;
	char tmp_rx_buf[16];

	ret = ops->enhance_frame_enable(edp_hw, enable);
	if (ret)
		return ret;
...
s32 edp_dpcd_set_lane_para(struct sunxi_edp_hw_desc *edp_hw, struct edp_tx_core *edp_core)
{
	...
	memset(tmp_rx_buf, 0, sizeof(tmp_tx_buf));
	ret = edp_hw_aux_read(edp_hw, DPCD_0101H, 1, &tmp_rx_buf[0]);
	if (ret)
		return ret;
